...
import { User, TutorData } from "@/types";
...
import { User as UserIcon, MapPin, Book, GraduationCap, Clock, Users } from "lucide-react";
...
export default function ProfileForm({ user, Tutor }: { user: User; Tutor: TutorData }) {
    const supabase = createClient();
    const [loading, setLoading] = useState(false);
    const [subjects, setSubjects] = useState<string[]>(
        Tutor.subjects?.filter(subject =>
            SUBJECT_OPTIONS.some(option => option.value === subject)
        ) || []
    );
    const [bio, setBio] = useState<string>(Tutor.bio || "");
    const [qualifications, setQualifications] = useState<string>(Tutor.qualifications || "");
    const [tutoring_approach, setTutorApproach] = useState<string>(Tutor.tutoring_approach || "");
    const [tutoring_experience, setTutorExperience] = useState<string>(Tutor.tutoring_experience || "");
    const [current_education, setCurrentEducation] = useState<string>(Tutor.current_education || "");
    const [location, setLocation] = useState<string>(Tutor.location || "");
